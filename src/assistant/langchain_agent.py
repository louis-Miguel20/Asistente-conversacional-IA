...
def detect_chunk_params(procedures_text: str) -> Tuple[int, int]:
    ...
    size = None
    overlap = None
    m_size = re.search(r"(chunk[_\s-]?size|tamañ[o]?\s+de\s+chunk|tamaño\s+del\s+bloque)\D?(\d{2,5})", text)
...
def build_splits(procedures_text: str, chunk_size: Optional[int] = None, chunk_overlap: Optional[int] = None) -> List[str]:
...
def answer_query(procedures_text: str, query: str, chunk_size: Optional[int] = None, chunk_overlap: Optional[int] = None, use_embeddings: bool = False) -> str:
